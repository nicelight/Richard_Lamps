      ...
      if ((ms - butMs) > 30) {
        ...
        if (digitalRead(BUTTON1) == ACTIVE) {
          ...
          if (digitalRead(BUTTON3) == ACTIVE) {
            // нажаты 1 и 3, идем на их взаимную обработку
            butcount = 0;
            autoBut = 70; //GO
#ifdef DEBUG
            Serial.println("But1 + But3");
#endif
          }// 1 & 4
          ...
          if (butcount < 40) {
            ...
            makelamps(20); //включим лампы и подсветку в новое состояние
            butcount = 0;
          ...
          if (butcount) {
            ...
            makelamps(255); //включим лампы и подсветку в новое состояние
          }
          autoBut = 99; // GO на ожидание отпускания кнопок
        }// no buttons pressed
      }//if ms
      ...
      if ((ms - butMs) > 100) {
        ...
        if ((digitalRead(BUTTON1) == ACTIVE) && (digitalRead(BUTTON4) == ACTIVE)) {
          if ( butcount < 30) {
            ...
            autoKlapan = 5; // заставляем автомат клапана реинициализироваться
#ifdef DEBUG
            Serial.print("klapanmode = ");
            Serial.print(klapanmode);
            if (klapanmode) Serial.println(" auto");
            else Serial.println(" manual");
#endif
            //Ринат: вернул отрисовку
            showLed();
            makelamps(255);
...
case 70:
      // одновременно нажаты 1 и 3, меняем master
      if ((ms - butMs) > 100) {
        butMs = ms;
        if ((digitalRead(BUTTON1) == ACTIVE) && (digitalRead(BUTTON3) == ACTIVE)) {
          if ( butcount < 30) {
            butcount ++;
          } else { // обе кнопки удерживались 3 секунды
            butcount = 0;
            master = !master; // ИНВЕРТИРУЕМ РЕЖИМ РАБОТЫ МАСТЕР-КНОПОК            
            //Ринат: вернул отрисовку
            showLed();
            makelamps(20);
      ...
      if ((ms - butMs) > 100) {
        ...
        if ((digitalRead(BUTTON2) == ACTIVE) && (digitalRead(BUTTON3) == ACTIVE)) {
          if ( butcount < 5) {
            ...
            butcount = 0;
            // если режим клапана ручной, переключим состояние клапана
            if (!klapanmode) {
              stateklapan = !stateklapan;
#ifdef DEBUG
              Serial.print("KLAPAN manually ");
              if (stateklapan) Serial.println("ON");
              else Serial.println("OFF");
#endif
            } // if klapanmode

            //отрисовка нажатия 2 и 3 кнопки
            showLed ();
            makelamps(255);  // возвращаем подсветку на исходное
...
void makelamps(byte dim) {
  ...
  if (statelamp1) {
    analogWrite(LEDLAMP1, dim);
  }
  else {
    analogWrite(LEDLAMP1, 0);
  }
  if (statelamp2) {
    analogWrite(LEDLAMP2, dim);
  }
  else {
    analogWrite(LEDLAMP2, 0);
  }
  if (statelamp3) {
    analogWrite(LEDLAMP3, dim);
  }
  else {
    analogWrite(LEDLAMP3, 0);
  }
  if (statergbwstrip) {
    analogWrite(LEDRGBWSTRIP, dim);
  }
  else {
    analogWrite(LEDRGBWSTRIP, 0);
  }
